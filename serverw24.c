...
*/
void crequest(int socket, char *handleby)
{
	struct dirent *dir;
	struct tm tmVar;
	...
	// sending the appropriate message to client if the request is to be handled by server or mirror 1 or mirror 2
	if (memcmp(handleby, "SERVER", strlen("SERVER")) == 0)
	{
		memcpy(message, "SERVER", strlen("SERVER"));
	}
	else if (memcmp(handleby, "MIRROR1", strlen("MIRROR1")) == 0)
	{
		memcpy(message, "MIRROR1", strlen("MIRROR1"));
	}
	else if (memcmp(handleby, "MIRROR2", strlen("MIRROR2")) == 0)
	{
		memcpy(message, "MIRROR2", strlen("MIRROR2"));
	}
	write(socket, message, strlen(message) + 1);
	// server closes it's socket in case it's not going to handle the request and exits
	if (memcmp(handleby, "SERVER", strlen("SERVER")) != 0)
	{
		close(socket);
		exit(0);
	...
	while (1)
	{
				...
				if (dir->d_type == DT_REG)
				{
					...
					if (strcmp(dir->d_name, ".") != 0 && strcmp(dir->d_name, "..") != 0)
					{
						...
						if (stat(path, &statbuf) == 0)
						{
							...
							if (statbuf.st_size >= size1 && statbuf.st_size <= size2 && strcmp(dir->d_name, "temp.tar.gz") != 0)
							{
								exists = 1;
		...
		if (strncmp(client_cmd, "w24ft", 5) == 0)
		{
			...
			while ((dir = readdir(d)) != NULL)
			{
				...
				if (dir->d_type == DT_REG)
				{
					...
					if (strcmp(dir->d_name, ".") != 0 && strcmp(dir->d_name, "..") != 0)
					{
						...
						if (((type1 != NULL) && !strncmp(ext, type1, strlen(type1))) || ((type2 != NULL) && !strncmp(ext, type2, strlen(type2))) || ((type3 != NULL) && !strncmp(ext, type3, strlen(type3))) && strcmp(dir->d_name, "temp.tar.gz") != 0)
						{
							exists = 1;
							strcat(file_list, dir->d_name);
							strcat(file_list, " ");
			...
			else
			{
				// create a file with 'x' in it and send it to client as a sign that required file/s were not found
				system("echo x > empty.txt");
				system("tar -cvf temp.tar empty.txt");
			...
			while ((dir = readdir(d)) != NULL)
			{
				...
				if (dir->d_type == DT_REG)
				{
					...
					if (stat(path, &statbuf) == 0)
					{
						...
						if (timefile <= timeVar && strcmp(dir->d_name, "temp.tar.gz") != 0)
						{
							file_present = 1;
							strcat(file_list, dir->d_name);
							strcat(file_list, " ");
...
int main(int argc, char const *argv[])
{
	...
	int reqcount = 0;
	bool handlebyserver = false, handlebymirror1 = false, handlebymirror2 = false;
	char handleby[10] = {'\0'};
	...
	while (1)
	{
		...
		reqcount += 1;
		// first 3 requests handled by server
		if (reqcount <= 3)
		{
			memset(handleby, '\0', 10);
			memcpy(handleby, "SERVER", strlen("SERVER"));
		}
		// next 3 requests handled by mirror 1
		else if (reqcount >= 4 && reqcount <= 6)
		{
			memset(handleby, '\0', 10);
			memcpy(handleby, "MIRROR1", strlen("MIRROR1"));
		}
		// next 3 requests handled by mirro 2
		else if (reqcount >= 7 && reqcount <= 9)
		{
			memset(handleby, '\0', 10);
			memcpy(handleby, "MIRROR2", strlen("MIRROR2"));
			handlebyserver = true;
		}
		// the next client requests cycle from server to mirror 1 to mirror 2 and so on
		else if (handlebyserver == true)
		{
			handlebyserver = false;
			handlebymirror1 = true;
			memset(handleby, '\0', 10);
			memcpy(handleby, "SERVER", strlen("SERVER"));
		}
		else if (handlebymirror1 == true)
		{
			handlebymirror1 = false;
			handlebymirror2 = true;
			memset(handleby, '\0', 10);
			memcpy(handleby, "MIRROR1", strlen("MIRROR1"));
		}
		else if (handlebymirror2 == true)
		{
			handlebymirror2 = false;
			handlebyserver = true;
			memset(handleby, '\0', 10);
			memcpy(handleby, "MIRROR2", strlen("MIRROR2"));
		}
		printf("Got a client <%d>, message from server\n", reqcount);
		// creating fork() for each client
		if (fork() == 0)
			crequest(connectedsocket, handleby);
